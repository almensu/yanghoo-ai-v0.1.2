...
const VideoEditor: React.FC<VideoEditorProps> = ({ 
  ...
  // 新片段
  const [newSegment, setNewSegment] = useState<VideoSegment>({
    start: 0,
    end: videoDuration || 0,
    label: '',
    description: ''
  ...
  const addSegment = () => {
    ...
    if (newSegment.end > (videoDuration || 0)) {
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {selectedSegments.map((segment, index) => (
                  <tr key={index}>
                    <td className="px-3 py-2 text-sm">{formatTime(segment.start)}</td>
                    <td className="px-3 py-2 text-sm">{formatTime(segment.end)}</td>
